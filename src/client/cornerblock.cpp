...
void CornerBlock::inCornerBlock(Player* player){
    switch(theBlockType)
    {
        case DORMITORY : CornerBlock::inDormitory(player);
            break;
        case THE61CALL : CornerBlock::in61Call(player);
            break;
        case BREAKSEMESTER : CornerBlock::inBreakSemester(player);
            break;
        case PLURALMAJOR : CornerBlock::inPluralMajor(player);
            break;
...
void CornerBlock::inDormitory(Player* player){
	...
	int takingenergy;
	switch(player->getCharacter_Type())
	{
